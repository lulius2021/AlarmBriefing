...
import {
  View, Text, ScrollView, StyleSheet, TextInput, Switch, TouchableOpacity, SafeAreaView,
} from 'react-native';
...
export function AlarmEditScreen({ navigation, route }: any) {
  const { addAlarm } = useAlarms();
  const [name, setName] = useState('Morgenwecker');
  const [hours, setHours] = useState('07');
  const [minutes, setMinutes] = useState('00');
  const [days, setDays] = useState<number[]>([1, 2, 3, 4, 5]); // Mo-Fr
  ...
  return (
    <SafeAreaView style={styles.container}>
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>Neuer Wecker</Text>
        <View style={{ width: 60 }} />
      </View>
      ...
      <ScrollView style={styles.scroll} contentContainerStyle={styles.scrollContent}>
        ...
        <View style={styles.section}>
          ...
          {snoozeEnabled && (
            <View style={styles.inlineRow}>
              <Text style={styles.inlineLabel}>Dauer (Minuten)</Text>
              <TextInput
        ...
        <GlowButton title="Wecker speichern" onPress={handleSave} size="lg" style={{ marginTop: 20 }} />
      </ScrollView>
    </SafeAreaView>
  );
}
